...
//     (left: Schema[A], right: Schema[A]) match {
//       case (s @ Schema.Transform(codec1, _, _), s2 @ Schema.Transform(codec2, _, _)) =>
//         equalsSchema(codec1.asInstanceOf[Schema[Any]], codec2.asInstanceOf[Schema[Any]])
...
//       case (Schema.Sequence(element1, _, _), Schema.Sequence(element2, _, _)) => equalsSchema(element1.asInstanceOf[Schema[Any]], element2.asInstanceOf[Schema[Any]])
...
//         lLabel == rLabel && lAnnotations.toSet == rAnnotations.toSet && equalsSchema(lSchema.asInstanceOf[Schema[Any]], rSchema.asInstanceOf[Schema[Any]])
//     }
